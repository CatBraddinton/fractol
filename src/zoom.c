...
static void	zoom(t_data *data, double mouse_x, double mouse_y, double zoom_f)
{
	double interpolation;

	interpolation = 1.0 / zoom_f;
...
void		zoom_image(int button, int x, int y, t_data *data)
{
	if (button == KEY_MOUSE_SCROLL_UP)
	{
		...
		data->params->zoom_f = data->params->zoom;
	}
	if (button == KEY_MOUSE_SCROLL_DOWN)
	{
		data->params->zoom -= 0.0001;
		data->params->zoom_f = 1.0 / data->params->zoom;
	}
	zoom(data, data->params->move.re,
			data->params->move.im, data->params->zoom_f);
